...
func NewBlock(s store.Store, logs *logger.Log, cfg *configs.Config) *Block {
	return &Block{
		S:    s,
		Logs: logs,
		Cfg:  cfg,
...
func (b *Block) LoadBlocks() (int64, error) {
	...
	for i := numBlock - 1; i >= numBlock-70; i-- {
		fmt.Println("load block - ", i)
	...
	fmt.Println("Len load block allTransInfo -  ", len(allTransInfo))
	fmt.Println("allTransInfo to Mongo - success!")

	b.Logs.WithFields(logrus.Fields{
		"Len load block allTransInfo -  ": len(allTransInfo),
		"Log_level":                       "Info",
	}).Info("Start loadblocks")
